...
function _log(logopt, logmsg, logcolorfg, logcolorbg) {
    ...
    logcolorfg = (`\x1b[${(((logcolorfg ?? 0) == 0 ? (casecolors.fg[(logopt ?? 0)]) : (logcolors[logcolorfg] ? logcolors[logcolorfg] : casecolors.fg[(logopt ?? 0)])))}m`);
    logcolorbg = (`\x1b[${(((logcolorbg ?? 0) == 0 ? (casecolors.bg[(logopt ?? 0)]) : (logcolors[logcolorbg] ? logcolors[logcolorbg] : casecolors[(logopt ?? 0)])))}m`);
    const logm = [` ${logcolorbg}${logcolorfg} ${date_()} \x1b[0m${logcolorbg}${logcolorfg} \x1b[0m > `, (logmsg?.message ?? logmsg)];
