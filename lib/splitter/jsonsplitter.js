...
class jsonsplitter {
    ...
    addAction = (action) => {
        ...
        i.splitterData[this.symbol].actions.push(action);
    };
    ...
    editKeySync = (keypath, value) => {
        ...
        if (this.getKeyFromObjectSync(objpath.object_main, ["keys", keypath_[1]]) === undefined) {
            newfile = this.addKeySync(keypath_, value);
        } else {
            newfile = i.splitterData[this.symbol].actualFiles[filepath] = this.addKeysToObjectSync(objpath.object, keypath_, value);
    ...
    editKeyAddSync = (keypath, value) => {
        ...
        if (this.getKeyFromObjectSync(objpath.object_main, ["keys", keypath_[1]]) === undefined) {
            ...
            newfile = i.splitterData[this.symbol].actualFiles[filepath] = this.addAppendKeysToObjectSync(objpath.object, keypath_, value);
    ...
    deleteKeySync = (keypath) => {
        ...
        if (keypath_.length > (objpath.object_main?.keynames?.length ?? this._options?.max_keys_in_file)) {
            file = i.splitterData[this.symbol].actualFiles[filepath] = this.deleteKeyFromObjectSync(file, keypath_);
    ...
    deleteKeyFromObjectSync = (object, keys) => {
        ...
        for (let i = 0; i < keys.length - 1; i++) {
            if (!(keys[i] in parentObj)) {
                this.emitError(Error(`key ${keys[i]} not in object`));
                return undefined;
            } else {
    ...
    deleteKeyFromObject = async (object, keys, noreject) => {
        return new Promise((resolve, reject) => {
            ...
            for (let i = 0; i < keys.length - 1; i++) {
                if (!(keys[i] in parentObj)) {
                    let err = Error(`key ${keys[i]} not in object`);
                    this.emitError(err);
                    if (noreject) return resolve(undefined);
