...
class jsonsplitter {
    ...
    deleteKeyFromObject = (object, keys) => {
        ...
        for (let i = 0; i < keys.length - 2; i++) {
            if (parentObj[keys[i]].hasOwnProperty(keys[i + 1])) {
    ...
    getMainKey = (keypath) => {
        ...
        if (!objpath?.object_main) return undefined;
        return objpath?.object_main[keypath[keypath.length - 1]];
    };
    ...
    getKey = (keypath) => {
        ...
        let filenum = this.getKeyFromObject(objpath.object_main, ["keys", objpath.keys[1]]);
        if (!(filenum ?? undefined) || isNaN(filenum)) {
            if (this._options.debug >= 5) console.error(new Error(`key(s) (${objpath.keys[1]}) not found in mainfile keys`));
            return undefined;
        };
        let filepath = objpath.path_main.replace(_mainreg, `${filenum}.json`);
        let file = this._files[filepath]?.();
        if (!file) return undefined;
        return this.getKeyFromObject(file, keypath);
    };
    ...
    addKey = (keypath, value) => {
        ...
        mainfile = this.addKeysToObject(mainfile, ["keys", objpath.keys[1]], mainfile.filenum);

        file = i.splitterData[this.symbol].actualFiles[filepath] = this.addKeysToObject(file, keypath, value);
    ...
    editKey = (keypath, value) => {
        ...
        let filenum = this.getKeyFromObject(mainfile, ["keys", objpath.keys[1]]);
        if (!filenum) {
            if (this._options.debug >= 5) console.error(new Error(`key (${objpath.keys[1]}) not found in mainfile`));
            return undefined;
        };
    ...
    deleteKey = (keypath) => {
        ...
        mainfile = this.deleteKeyFromObject(mainfile, ["keys", objpath.keys[1]]);

        file = i.splitterData[this.symbol].actualFiles[filepath] = this.deleteKeyFromObject(file, keypath);
