...
class jsonsplitter {
    ...
    getFileByKeys = async (keypath) => {
        return new Promise((resolve, reject) => {
            ...
            for (let i = 0; i < keypath.length; i++) {
                ...
                if (filteredkeypath && filteredkeypath.length > 0) {
                    p_main = filteredkeypath[0];
                    o_main = this._mainfiles[p_main];
                    filenum = o_main().filenum;
                    if (o_main().keys[keypath[i + 1]]) {
                        filenum = o_main().keys[keypath[i + 1]];
                        keyfound = true;
                    } else {
                        keyfound = false;
                    }
            ...
            return resolve({
                get path() { return p },
                get path_main() { return p_main },
                get object_main() { return o_main?.() },
                get object() { return o?.() },
    ...
    getKey = async (keypath, noreject) => {
        return new Promise(async (resolve, reject) => {
            ...
            this.getKeyFromObject(file, keypath_, noreject)
                .then(resolve)
                .catch(e => {
    ...
    addKey = async (keypath, value, noreject) => {
        return new Promise(async (resolve, reject) => {
            ...
            if (mainfile.filekeynum >= this._options.max_keys_in_file) {
                ...
                _wf(this.symbol, filepath, this.createObjectFromKeys(mainfile.keynames, {}));
                file = i.splitterData[this.symbol].actualFiles[filepath];
            ...
            this.addKeysToObject(mainfile, ["keys", objpath.keys[1]], mainfile.filenum)
                .then(newmainfile => {
    ...
    editKey = async (keypath, value, noreject) => {
        return new Promise(async (resolve, reject) => {
            ...
            // this.getKeyFromObject(file, this.convertToArray(keypath_.slice(0, keypath_.length - 2)))
            // .then(addobj => {
            // this.addKeysToObject(addobj, this.convertToArray(keypath_[keypath_.length - 1]), value)
            //     .then(added => {
            //         this.addKeysToObject(file, this.convertToArray(keypath_.slice(0, keypath_.length - 2)), added)
            //             .then(newfile => {
            //                 file = i.splitterData[this.symbol].actualFiles[filepath] = newfile;
            //                 if (!mainfile.hasChanges) mainfile.hasChanges = [];
            //                 if (!mainfile.hasChanges.includes(filepath)) mainfile.hasChanges.push(filepath);
            ...
            //     })
            //     .catch(e => {
            //         let err = Error(`could not add (first) key (path: ${keypath_[keypath_.length - 1]}) to file object (keypath: ${keypath}, noreject: ${noreject})`, { "cause": e });
            //         this.emitError(err);
            //         if (noreject) return resolve(undefined);
            //         return reject(err);
            //     });
            // })
            // .catch(e => {
            //     let err = Error(`could not get key (path: ${keypath_.slice(0, keypath_.length - 1)}) from file (keypath: ${keypath}, noreject: ${noreject})`, { "cause": e });
            //     this.emitError(err);
            //     if (noreject) return resolve(undefined);
            //     return reject(err);
            // });
    ...
    deleteKey = async (keypath, noreject) => {
        return new Promise(async (resolve, reject) => {
            ...
            let objpath = await this.getFileByKeys(keypath_);
            if (!objpath.object_main) {
                let err = Error(`objpath.object_main is undefined (keypath: ${keypah_}, noreject: ${noreject})`);
                this.emitError(err);
                if (noreject) return resolve(undefined);
                return reject(err);
            };
            ...
            let mainkeypath = ["keys", keypath_[1]];
            this.getMainKey(mainkeypath)
                .then(filenum => {
                    let filepath = objpath.path_main.replace(_mainreg, `${filenum}.json`);
                    let file = objpath.object;

                    this.deleteKeyFromObject(mainfile, ["keys", objpath.keys[1]])
                        .then(newmainfile => {
                            mainfile = newmainfile
                            this.deleteKeyFromObject(file, keypath)
                                .then(newfile => {
                                    file = i.splitterData[this.symbol].actualFiles[filepath] = newfile;
                        ...
                        .catch(e => {
                            let err = Error(`could not delete key ${objpath.keys[1]} from mainkeys (keypath: ${keypath}, noreject: ${noreject})`, { "cause": e });
                            this.emitError(err);
                            if (noreject) return resolve(undefined);
                            return reject(err);
                        });
                ...
                .catch(e => {
                    let err = Error(`could not get key ${objpath.keys[1]} from mainfile keys (keypath: ${keypath}, noreject: ${noreject})`, { "cause": e });
                    this.emitError(err);
                    if (noreject) return resolve(undefined);
                    return reject(err);
                })
