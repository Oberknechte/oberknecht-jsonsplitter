...
module.exports = class jsonsplitter {
    ...
    getKey = (keypath) => {
        ...
        if (!objpath) {
            if(this._options.debug) console.error(new Error(`dir (${objpath}) not in mainfiles`));
            return undefined;
        };

        let filenum = this.getKeyFromObject(objpath.object_main, ["keys", ...objpath.keys.slice(1, 2)]);
        if (!(filenum ?? undefined) && isNaN(filenum)) {
            if(this._options.debug) console.error(new Error(`key(s) (${objpath.keys.slice(1, 2)}) not found in mainfile keys`));
            return undefined;
        };
    ...
    editKey = (keypath, value) => {
        ...
        if (!filenum) {
            if(this._options.debug) console.error(new Error(`key (${keypath.slice(1, 2)}) not found in mainfile`));
            return undefined;
        };
        ...
        if (!mainfile.hasChanges) mainfile.hasChanges = [];
        if (!mainfile.hasChanges.includes(filepath)) mainfile.hasChanges.push(filepath);
        let newkey = this.addKeysToObject(this.getKey(this.convertToArray(keypath.slice(0, keypath.length-2))), this.convertToArray(keypath.slice(keypath.length-2)), value);
        file = i.splitterData[this.symbol].files[filepath] = this.addKeysToObject(file, keypath.slice(0, keypath.length-2), newkey);
