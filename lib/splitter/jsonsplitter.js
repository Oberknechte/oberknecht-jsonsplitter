...
class jsonsplitter {
    ...
    addKeysToObjectSync = async (object, keys, value) => {
        keys = this.convertToArray(keys);
    ...
    addAppendKeysToObjectSync = async (object, keys, value) => {
        keys = this.convertToArray(keys);

        let oldvalue = this.getKeyFromObjectSync(object, keys);

        this.addKeysToObjectSync(object, keys, (oldvalue ? (oldvalue + value) : value));

        return object;
    };

    getKeyFromObjectSync = async (object, keys) => {
        let value = object;
    ...
    deleteKeyFromObjectSync = async (object, keys) => {
        let parentObj = object;
        for (let i = 0; i < keys.length - 1; i++) {
    ...
    getMainKeySync = (keypath) => {
        ...
        this.getKeyFromObject(objpath.object_main, keypath.slice(1))
            ...
            .catch(e => {
                let err = Error(`could not get key ${keypath[keypath.length - 1]} from mainfile keys (keypath: ${keypath})`, { "cause": e });
                this.emitError(err);
                return undefined;
            })
    };
    ...
    addKeySync = async (keypath, value) => {
        let keypath_ = this.convertToArray(keypath);
        let objpath = this.getFileByKeys(keypath_);

        if (!objpath.object_main?.num) await this.create(this.addKeysToObjectSync({}, keypath_, value));
        ...
        if (this._options.silent?._all || this._options.silent?.addKey) return true;
        return newfile;
    };

    editKeySync = async (keypath, value) => {
        let keypath_ = this.convertToArray(keypath);
        let objpath = this.getFileByKeys(keypath_);
        ...
        let newfile = this.addKeysToObject(objpath.object, keypath_, value);
        ...
        if (this._options.silent?._all || this._options.silent?.editKey) return true;
        return newfile;
    };

    editKeyAddSync = async (keypath, value) => {
        let keypath_ = this.convertToArray(keypath);
        let objpath = this.getFileByKeys(keypath_);

        if (!objpath.keyfound) {
            let err = Error(`objpath.keyfound is false (keypath: ${keypath})`);
            this.emitError(err);
            return undefined;
        };
        ...
        if (this._options.silent?._all || this._options.silent?.editKeyAdd) return true;
        return newfile;
    };

    deleteKeySync = async (keypath) => {
        let keypath_ = this.convertToArray(keypath);
        let objpath = this.getFileByKeys(keypath_);
        ...
        if (keypath_.length > (objpath.object_main?.keynames?.length ?? i.splitterData[this.symbol].max_keys_in_file)) {
            file = this.deleteKeyFromObjectSync(file, keypath)
            ...
            if (this._options.silent?._all || this._options.silent?.deleteKey) return true;
            return file;
        } else {
            if (!objpath.dirpath) {
                let err = Error(`could not get maindirectory of specified keypath (keypath: ${keypath})`, { "cause": e });
                this.emitError(err);
                return undefined;
            };
    ...
    addAppendKeysToObject = async (object, keys, value) => {
        return new Promise(async (resolve, reject) => {
            ...
            let oldvalue = await this.getKeyFromObject(object, keys, true);

            await this.addKeysToObject(object, keys, (oldvalue ? (oldvalue + value) : value));
