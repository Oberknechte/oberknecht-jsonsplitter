...
class jsonsplitter {
    ...
    addKeySync = (keypath, value) => {
        ...
        if (keypath_.length == (objpath.object_main.keynames.length + 1) && !this.getKeyFromObjectSync(objpath.object_main, ["keys", objpath.keys[1]])) {
    ...
    editKeySync = (keypath, value) => {
        ...
        let newfile = i.splitterData[this.symbol].actualFiles[filepath] = this.addKeysToObjectSync(objpath.object, keypath_, value);
    ...
    deleteKeySync = (keypath) => {
        ...
        if (keypath_.length > (objpath.object_main?.keynames?.length ?? i.splitterData[this.symbol]._options?.max_keys_in_file)) {
            file = i.splitterData[this.symbol].actualFiles[filepath] = this.deleteKeyFromObjectSync(file, keypath);
