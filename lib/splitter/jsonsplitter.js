...
class jsonsplitter {
    ...
    getFileByKeys = async (keypath) => {
        return new Promise((resolve, reject) => {
            let r = { path: undefined, path_main: undefined, object: undefined, object_main: undefined, keyfound: false, filenum: undefined };
    ...
    addKey = async (keypath, value, noreject) => {
        return new Promise(async (resolve, reject) => {
            ...
            if (!objpath.object_main) {
                ...
                // if (noreject) return resolve(undefined);
                // return reject(err);
                await this.create(this.createObjectFromKeys(keypath_, value));
            };
    ...
    deleteKey = async (keypath, noreject) => {
        return new Promise(async (resolve, reject) => {
            ...
            this.getMainKey(mainkeypath, noreject)
                .then(filenum => {
                    let filepath = objpath.path_main.replace(_mainreg, `${filenum}.json`);
                    let file = objpath.object;

                    if (keypath_.length > i.splitterData[this.symbol].max_keys_in_file) {
                        ...
                        this.deleteKeyFromObject(objpath.object_main, mainkeypath)
                            .then(newmainfile => {
                                i.splitterData[this.symbol].actualMainFiles[mainpath] = newmainfile;

                                return resolve(newmainfile);
                            })
                            .catch(e => {
