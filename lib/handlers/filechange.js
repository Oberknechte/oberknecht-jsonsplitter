...
module.exports = (sym, interval) => {
    function filechange() {
        let changed_files = 0;
        if(i.splitterData[sym]._options?.debug) _log(1, `[JSONSPLITTER] [FILECHANGE] Executed`);
        ...
        if(i.splitterData[sym]._options?.debug) _log(1, `[JSONSPLITTER] [FILECHANGE] Changed ${changed_files} files`);
    };
